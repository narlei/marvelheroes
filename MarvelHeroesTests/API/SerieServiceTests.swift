...
class SerieServiceTests: XCTestCase {
    
    let provider = MoyaProvider<SerieService>(stubClosure: MoyaProvider.immediatelyStub)
    
    func testList() {
        provider.request(.detail(id: 0)) { (response) in
            switch response {
            case .success(let result):
                if let arrayPerson = result.mapMappable(object: Serie.self) {
    ...
    func testListError() {
        ...
        let arrayPerson = result.mapMappable(object: Serie.self)
